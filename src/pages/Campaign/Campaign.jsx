...
const Campaign = () => {
    ...
    if (campaign?._id) {
        ...
        return (
            <>
                <Helmet>
                    <title>{campaign?.title || "Not Found | Razels"}</title>
                        ...
                        <section className='w-full fixed h-screen items-center flex justify-center z-[999] bg-[#1b1b1b43] px-10     py-40'>
                            <div  className="z-[999999] flex space-y-5 flex-col items-center p-10 justify-center w-full lg:w-5/12 2xl:w-[500px] bg-white  dark:bg-slate-800 dark:border-border-1 rounded-2xl">
                                <div className='w-full flex justify-end'>
                                    <IoMdClose onClick={() => setdonated(false)} size={26} className="dark:hover:bg-slate-700 hover:bg-slate-200 cursor-pointer rounded-md" />
                                </div>
                                <JackInTheBox>
                                    <img width={100} src={Tick} alt="Tick" />
                                </JackInTheBox>
                                <div className='text-center'>
                                    <h2 className='text-2xl dark:text-white font-medium'>Donation Succesfull</h2>
                                    <p>See your donation here</p>
                                    <Link onClick={() => setdonated(false)} to={`../mydonations`}>
                                        <button className='mt-5 dark:bg-slate-700 px-5 py-2 border dark:border-border-1 rounded-[70px] hover:rounded-xl transition-all'>See See Donation</button>
                                    </Link>
                                </div>
                            </div>
                        </section>
                ...
                <section className="flex justify-center">
                    <div className="w-primary flex justify-between flex-col lg:flex-row items-start py-24 px-5 gap-5">
                        ...
                        <form onSubmit={handelDonation} className="w-full mt-10 lg:w-4/12 border p-10 dark:border-border-1 space-y-5 rounded-3xl block lg:hidden">
                                {isDeadlineExpired 
                                    ? (
                                        <Alert className="bg-[#f4c0c051]" variant="destructive">
                                            <AlertCircle className="h-6 w-6" />
                                            <AlertTitle className="ml-2 !font-semibold text-lg">Expired</AlertTitle>
                                            <AlertDescription className="ml-2 dark:text-white">
                                                This campaign expired on {moment(campaign?.deadline).format('dddd, MMMM DD, YYYY')}.
                                            </AlertDescription>
                                        </Alert>
                                    ):(
                                        <Alert className="!bg-transparent border dark:border-border-1">
                                            ...
                                            <AlertDescription>
                                                This Campaign Deadline Is <strong>{moment(campaign?.deadline).format('dddd, MMMM DD, YYYY')}</strong>
                                            </AlertDescription>
                                        </Alert>
                                    )
                                }
                                <div>
                                    <h3 className='font-medium pb-1 dark:text-white'>Minimum Ammount: {campaign?.minAmmount}$</h3>
                                    <input name="doyaloBektirDan" defaultValue={campaign?.minAmmount} type="number" className='w-full bg-transparent px-5 py-2 border dark:border-border-1 rounded-lg focus:ring-2 ring-[#2580ff63]' />
                                </div>
                                ...
                                <button disabled={isDeadlineExpired} className={`flex items-center gap-2 w-full bg-red-500 hover:bg-red-700 font-medium text-center text-white justify-center px-5 py-3 rounded-xl ${isDeadlineExpired && 'opacity-35'}`}>
                                    Donate
                                    {
                                        !isLoadingIcon 
                                            ? <FaHeart />
                            ...
                            <div className="mt-10 p-5 px-8 dark:bg-bg-1 w-full dark:border-border-1 border gap-x-10 gap-y-5 flex justify-between items-center rounded-xl flex-wrap">
                                <div className="flex items-center gap-4">
                                    ...
                                    />
                                    <div>
                                        <h4 className="text-xl dark:text-white">{campaign?.userName}</h4>
                            ...
                            <div className="mt-5 p-5 dark:bg-bg-1 w-full dark:border-border-1 border rounded-xl space-y-3">
                                <h2 className="dark:text-white text-3xl md:text-3xl font-semibold">{campaign?.title}</h2>
                        ...
                        <form onSubmit={handelDonation} className="w-full lg:w-4/12 border p-10 dark:border-border-1 space-y-5 rounded-3xl hidden lg:block">
                            {isDeadlineExpired 
                                ? (
                                    <Alert className="bg-[#f4c0c051]" variant="destructive">
                                        <AlertCircle className="h-6 w-6" />
