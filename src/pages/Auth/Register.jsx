        ...
        createUserWithEmailAndPassword(auth, email, password)
            .then((user) => {
                updateProfile(auth.currentUser, {
                    displayName: name, photoURL: Avatar.url
                }).then((user) => {
                    setauthInfo({
                        ...authInfo,
                        displayName: name,
                ...
                <div className="w-primary py-60 lg:h-screen px-5 flex justify-center items-center">
                    ...
                    >
                        <div className="w-full px-5 pt-2 flex flex-col gap-5">
                            ...
                            <input ref={uploadAvatar} onChange={handelUploadImage} id="avtar" type="file" className="hidden" />
                            <label  htmlFor="avtar" className="w-full p-5 border flex items-center gap-5 cursor-pointer text-lg rounded-xl dark:border-border-1 dark:bg-slate-800 bg-slate-100">
                                {isUploading ? <PiSpinnerGapThin size={25} className="animate-spin"/> : Avatar?.url ? <IoCheckmarkOutline size={25}/> : <BsUpload size={25} />} {isLoading ? 'Uploading...' : Avatar?.url ? 'Upload Succes' : 'upload Image'}
                            </label>
                            ...
                            <button disabled={isLoading} type="submit" className="w-full py-2 flex justify-center gap-2 px-5 mt-2 bg-sky-600 hover:bg-sky-800 rounded-xl text-white">
                                {
                                    isLoading
                            ...
                            <SocialSignIn />
                            <span className="w-full h-[1px] dark:bg-border-1 bg-border-2"></span>
                            <p className="text-center">Already have an account <Link className="text-sky-600" to={'/auth/login'}>Login</Link></p>
                        </div>
                    </Form>
                </div>
            </section>
