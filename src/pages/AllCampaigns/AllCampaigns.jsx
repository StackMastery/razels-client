...
const AllCampaigns = () => {
  ...
  const serachHandel = () => {
    ...
    const filterData = storedData.filter((filter) => filter.title.toLowerCase().includes(searchValue))
  ...
  return (
    <>
      <Helmet>
        <title>All Campaigns | Razels</title>
      </Helmet>
          ...
          <div className="w-full p-7 md:p-10 border bg-pattern dark:border-border-1 flex justify-between items-center rounded-3xl flex-col lg:flex-row">
            <div data-aos="fade-up" className="space-y-2 z-20">
              ...
              <p className="text-sm md:text-base">
                View all active and expired campaigns on Razels Des. Discover
                projects seeking
        ...
        <div className="w-primary flex justify-center px-5">
          <div className="w-full flex items-center justify-between border p-5 rounded-xl gap-5 flex-wrap dark:border-border-1">
            <div className="flex items-center gap-5 w-8/12 flex-grow md:flex-grow-0 lg:w-4/12">
                <input className="w-full bg-transparent outline-none px-5 py-2 border dark:border-border-1 rounded-xl focus:ring-2 dark:ring-border-1 ring-border-2 " onChange={serachHandel} ref={serachRef} placeholder="Type Something To Search" name="search" />
            </div>
            <div className="w-[200px] flex-grow md:flex-grow-0">
                <button onClick={() => {setsorted(!sorted) ;sortHandel();}} className="w-full dark:bg-slate-800 py-2 rounded-lg border dark:border-border-1 flex items-center gap-3 justify-center">
                    Sort by Price
                    {sorted
                        ? <GoSortDesc size={20}/>
                        ...
                        return (
                            ...
                            >
                                <TableCell className="font-medium">
                                    {camp.title.slice(0,)}
                                    {verifyDeadline({deadline: moment(camp?.deadline).format("YYYY-MM-DD")}) === 1 ? <span className="bg-sky-600 text-white px-3 py-[2px] rounded-full ml-2">Published</span> : <span className="bg-red-600 text-white px-3 py-[2px] rounded-full ml-2">Expired</span>}
                                </TableCell>
                                ...
                                <TableCell>
                                    {moment(camp?.deadline).format('dddd, MMMM D, YYYY')}
                                </TableCell>
                                <TableCell>{camp.category.split("-").join(" ")}</TableCell>
                                <TableCell className="text-right">
                                    <Link to={`../campaign/${camp?._id}`}>
                                      <button className="hover:underline hover:text-sky-600">
                                        View Details
                                      </button>
                                    </Link>
                                </TableCell>
