...
const MyCampaign = () => {
    ...
    const fethData = () => {
        fetch(`${import.meta.env.VITE_BACKEND_URL}/api/allcampaigns?ownerUID=${authInfo?.uid}`)
            .then(res => res.json())
            .then((data) => {
                setmyCampaigns(data)
    ...
    const handelDelete = (e) => {
        ...
        fetch(`${import.meta.env.VITE_BACKEND_URL}/api/deletecampaign`, requestOptions)
    ...
    return (
        <>
            <Helmet>
                <title>My Campaigns | Razels</title>
            </Helmet>
            {
                deletModal && (
                    ...
                    <div  className="flex border space-y-5 flex-col items-center p-10 justify-center w-full lg:w-5/12 2xl:w-[500px] bg-white  dark:bg-slate-800 dark:border-border-1 rounded-2xl">
                            <div className='w-full flex justify-end z-10'>
                                <IoMdClose onClick={() => setdeletModal(false)} size={26} className="dark:hover:bg-slate-700 hover:bg-slate-200 cursor-pointer rounded-md" />
                            </div>
                            <div className="w-full space-y-2">
                                <h2 className="dark:text-white text-2xl -mt-10 font-medium">Want To Delete?</h2>
                                <p>If you want to delet this campaign copy this and type <button onClick={() => copyToClipboard(deletModal?._id?.slice(0, 10))} className="dark:text-white text-red-600 font-bold">{deletModal?._id?.slice(0, 10)}</button></p>
                                <form className="space-y-3 pt-3" onSubmit={handelDelete}>
                                    <input name="deletId" placeholder="Type Campaign Id"  type="text" className='w-full bg-transparent px-5 py-2 border dark:border-border-1 rounded-lg focus:ring-2 ring-[#2580ff63]' />
                                    <button className="bg-red-600 px-5 py-2 text-white rounded-lg hover:rounded-[70px] transition-all">Delete</button>
                                </form>
                            </div>
                        </div>
                    </section>
                ...
                <div className="w-primary pt-28 px-5 overflow-y-hidden">
                    ...
                    <div data-aos="fade-up" className="space-y-2 z-20">
                        ...
                        <p className="text-sm md:text-base">
                            You Can <strong> Edit, Delete, Preview</strong> Your Created Campaigns
                        </p>
                    </div>
                    <JackInTheBox>
                        <img
                ...
                <div className="w-primary px-5 pt-10 pb-20 flex justify-center items-center overflow-x-scroll">
                    <ScrollArea className="w-full">
                        <Table className="dark:!text-text-1 w-primary lg:w-full">
                            <TableCaption className="dark:text-text-1 text-text-2 dark:bg-slate-800 px-5 py-2 rounded-3xl border dark:border-border-1">A list of your added campaigns.</TableCaption>
                            <TableHeader>
                                    ...
                                    isLoadingData
                                        ? (
                                            Array.from({length: 8}).map((_, index) => (
                                                <TableRow key={index} className="dark:hover:bg-slate-950 dark:border-border-1 cursor-pointer">
                                        ...
                                        : (
                                            myCampaigns.length > 0
                                                ? (
                                                    myCampaigns && myCampaigns.map((camps, index) => (
                                                        <TableRow key={index} className="dark:hover:bg-slate-950 dark:border-border-1 cursor-pointer transition-none">
                                                            <TableCell className="font-medium">{camps?.title?.slice(0, 40)}{camps?.title.length > 39 && '...'}{verifyDeadline({deadline: moment(camps?.deadline).format("YYYY-MM-DD")}) === 1 ? <span className="bg-sky-600 text-white px-3 py-[2px] rounded-full ml-2">Published</span> : <span className="bg-red-600 text-white px-3 py-[2px] rounded-full ml-2">Expired</span>}</TableCell>
                                                            <TableCell>{camps?.minAmmount}$</TableCell>
                                                            <TableCell>{moment(camps?.createdAt).format('dddd, MMMM D, YYYY')}</TableCell>
                                                            <TableCell>
                                                            ...
                                                            <TableCell>
                                                                <button onClick={() => setdeletModal(camps)} className="p-1 rounded-md flex justify-center bg-neutral-200 dark:bg-slate-800 hover:bg-red-400 hover:text-white">
                                                                    <CiTrash size={20} />
                                                                </button>
                                                            </TableCell>      
                                                            <TableCell className="text-right">
                                                                <Link to={`../campaign/${camps?._id}`}>
                                                                    <button className="hover:underline hover:text-sky-600">Veiw Details</button>
                                                                </Link>
                                                            </TableCell>
                                                        </TableRow>
                                                    ))
