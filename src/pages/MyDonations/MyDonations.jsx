...
const MyDonations = () => {
  ...
  useEffect(() => {
    ...
    fetch(
      `${import.meta.env.VITE_BACKEND_URL}/api/mydonations?ownerUID=${authInfo?.uid}`,
      requestOptions,
    )
    ...
    return (
      <>
        <Helmet>
          <title>My Donations | Razels</title>
        </Helmet>
                ...
                ? (
                  ...
                  : (
                    <div className="col-span-12 space-y-3 flex flex-col items-start bg-slate-100 border dark:border-border-1 dark:bg-slate-800 p-10 rounded-xl">
                       <h2 className="text-3xl dark:text-white">Your No Donation History Was Found</h2>
                       <Link to={`../campaigns`}>
                          <button className="bg-sky-600 text-white px-5 rounded-md py-1">Donate campaign</button>
                       </Link>
                    </div>
                  )
                )
