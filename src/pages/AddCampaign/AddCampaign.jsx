...
const AddCampaign = () => {
    ...
    const handelAddCampaign = (e) => {
        ...
        urlencoded.append("campDeadline", moment(campDeadline, 'YYYY-MM-DD').toISOString());
        ...
        fetch(`${import.meta.env.VITE_BACKEND_URL}/api/campaign/add`, requestOptions)
    ...
    const handelUploadImage = async () => {
        ...
        dataFile.append("upload_preset", "razels")
    ...
    return (
        <>
            <Helmet>
                <title>Add New Campaign | Razels</title>
            </Helmet>
            {
                succesModal && (
                    <section className='w-full fixed h-screen items-center flex justify-center z-[9999999999999999] bg-[#1b1b1b43] px-10     py-40'>
                        <div  className="flex space-y-5 flex-col items-center p-10 justify-center w-full lg:w-5/12 2xl:w-[500px] bg-white  dark:bg-slate-800 dark:border-border-1 rounded-2xl">
                            <div className='w-full flex justify-end'>
                                <IoMdClose onClick={() => setsuccesModal(false)} size={26} className="dark:hover:bg-slate-700 hover:bg-slate-200 cursor-pointer rounded-md" />
                            </div>
                            <JackInTheBox>
                                <img width={100} src={Tick} alt="Tick" />
                            </JackInTheBox>
                            <div className='text-center'>
                                <h2 className='text-2xl dark:text-white font-medium'>Campaign Added Succes</h2>
                                <p>Visit you added campaign</p>
                                <Link onClick={() => setsuccesModal(false)} to={`../campaign/${succesModal}`}>
                                    <button className='mt-5 dark:bg-slate-700 px-5 py-2 border dark:border-border-1 rounded-[70px] hover:rounded-xl transition-all'>See Your Campaign</button>
                                </Link>
                            </div>
                        </div>
                    </section>
                ...
                <div className="w-primary pt-24 px-5 overflow-y-hidden">
                    <div className="w-full p-7 md:p-10 border bg-pattern dark:border-border-1 flex justify-between items-center rounded-3xl flex-col lg:flex-row">
                        <div data-aos="fade-up" className="space-y-2 z-20">
                            <h2 className="dark:text-white text-3xl md:text-4xl font-semibold">Crowdfunding Made Easy</h2>
                            <p className='text-sm md:text-base'>Discover how to launch a successful crowdfunding campaign! Engage your audience,<br className="hidden md:block"/> tell your story, and achieve your funding goals to turn your vision into reality</p>
                        </div>
                        <JackInTheBox>
                            <img width={100} src={PowerImage} alt="Power" className='animate-pulse opacity-40 lg:opacity-100 -mt-32 lg:m-0 z-10'/>
                        </JackInTheBox>
                    </div>
                </div>
            </section>
            <section className='flex justify-center'>
                <form onSubmit={handelAddCampaign} className='w-primary flex flex-col-reverse lg:flex-row px-5 py-10 gap-10'>
                    <div className='w-full lg:w-8/12 p-7 space-y-5 md:p-10 border dark:border-border-1 rounded-3xl'>
                        <div>
                            <h3 className='font-medium pb-1 dark:text-white'>Campaign Title</h3>
                            <input placeholder="Enter A Title" name='campTitle' type="text" className='w-full bg-transparent px-5 py-2 border dark:border-border-1 rounded-lg focus:ring-2 ring-[#2580ff63]' />
                        </div>
                        <div>
                            <h3 className='font-medium pb-1 dark:text-white'>Campaign Type </h3>
                            <Select name="campCategory">
                                ...
                                <SelectContent>
                                    <SelectGroup>
                                        ...
                                        <SelectItem value="creative-arts-&-entertainment">Creative Arts & Entertainment</SelectItem>
                        ...
                        <div className='flex justify-between gap-5 w-full flex-col sm:flex-row'>
                            <div className='sm:w-6/12 flex-grow'>
                                <h3 className='font-medium pb-1 dark:text-white'>Min Ammount</h3>
                                <input placeholder="0+" name='minAmmount' type="number" className='w-full bg-transparent px-5 py-2 border dark:border-border-1 rounded-lg focus:ring-2 ring-[#2580ff63]' />
                            </div>
                            <div className='sm:w-6/12 flex-grow'>
                                <h3 className='font-medium pb-1 dark:text-white'>Deadline</h3>
                                <input placeholder="0+" name='campDeadline' type="date" className='w-full cursor-text bg-transparent px-5 py-2 border dark:border-border-1 rounded-lg focus:ring-2 ring-[#2580ff63]' />
                            </div>
                        </div>
                        <div className='flex justify-between gap-5 w-full flex-col sm:flex-row'>
                            <div className='sm:w-6/12 flex-grow'>
                                <h3 className='font-medium pb-1 dark:text-white'>Author Name</h3>
                            ...
                            <div className='sm:w-6/12 flex-grow'>
                                <h3 className='font-medium pb-1 dark:text-white'>Author Email</h3>
                    ...
                    <div className='w-full lg:w-4/12'>
                        <input disabled={isUploading} ref={campThumbRef} onChange={handelUploadImage} name='campThumb' id='thumb' className='hidden' type="file" />
                        <label htmlFor='thumb' className='w-full cursor-pointer p-20 space-y-3 border dark:border-border-1 rounded-xl flex justify-center flex-col items-center'>
                            <div>
                                {
                                    isUploading
                        ...
                        {
                            campThumb?.url && (
                                <div style={{backgroundImage: `url('${campThumb?.url}')`}} className="w-full bg-cover bg-center p-40 space-y-3 border dark:border-border-1 rounded-xl mt-5">
                                </div>
                            )
                        }
                    </div>
